...
//        dd($debutMois);
    $mois = $request->query("mois");
        $utilisateurs = DB::table('users')->RightJoin("events","events.idUser","users.id")->where("idUser", "=", Auth::user()->id)->RightJoin("historique_essences","historique_essences.userId","users.id")->whereBetween("end",[$debutMois,$FinMois])->orderBy("start", "asc")->get();
// dd($utilisateurs);
        ...
        // $moisencours = Carbon::createFromFormat("m/d/Y");
            ...
            $pdf = PDF::loadView('pdf.PDFnotesdefrais',compact("utilisateurs","mois"))->setPaper('a4', 'landscape');
