...
<table class="tablepdf">
    ...
    <th class="TH-table text-center BGgreen" style="border-left: 2px solid black">KM (ici mettre valeur des cheveux
        fiscaux)
    </th>
    ...
    @foreach ($utilisateurs as $utilisateur)
        <tr>
            ...
            @php
                ...
                $totalTVA10 =round(($utilisateur->repas + $utilisateur->hotel)*0.1/1.1,2) + $totalTVA10;
            @endphp
            <td class="col-table text-center BGjour" style="border-left: 2px solid black">{{$datedebut[0] }}
                à {{$datefin[0]}}</td>
            <td class="col-table text-center" style="border-left: 2px solid black">{{$utilisateur->title}}</td>
            <td class="col-table text-center">{{$utilisateur->ville}}</td>
            <td class="col-table text-center">{{$utilisateur->code_postal}}</td>
            <td class="col-table text-center" style="border-left: 2px solid black">{{$utilisateur->peage}} €</td>
            <td class="col-table text-center">{{$utilisateur->parking}} €</td>
            <td class="col-table text-center">{{$utilisateur->essence}} €</td>
            <td class="col-table text-center">{{$utilisateur->divers}} €</td>
            <td class="col-table text-center"
                style="border-left: 2px solid black">{{round(($utilisateur->divers+$utilisateur->peage+$utilisateur->essence+$utilisateur->parking)/1.2*0.2,2)}}
                €
            </td>
            <td class="col-table text-center" style="border-left: 2px solid black">{{$utilisateur->repas}} €</td>
            <td class="col-table text-center">{{$utilisateur->hotel}} €</td>
            <td class="col-table text-center">{{round(($utilisateur->repas + $utilisateur->hotel)*0.1/1.1,2)}} €</td>

            <td class="col-table text-center" style="border-left: 2px solid black">{{$utilisateur->kilometrage}}</td>
            <td class="col-table text-center"
    ...
    <tr>
        ...
        <td class="TD-table text-center" colspan="1">{{$totalKilometres * $utilisateurs[0]->chevauxFiscaux}} €</td>
        //a modifier pour dynamiser par date
        <td class="TD-table text-center BGnuit" colspan="1"></td>
